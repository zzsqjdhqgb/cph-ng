...
最左边的按钮用于**创建一个单独的测试样例**。您需要手动输入输入数据和预期答案（输出）。

![添加一个测试样例](images/addTestCase.png){ width=800 loading=lazy }
/// caption
添加一个测试样例
///

!!! tip "批量导入测试样例"
    CPH-NG 还支持**从压缩文件或特定目录加载测试样例**，这是管理大型数据集的绝佳选择。详细信息可以参考 [加载测试样例](./features/load-test-cases.md)。

## 运行测试样例

- 要运行**单个测试样例**，请单击该测试样例旁边的绿色播放按钮。

- 要**同时运行所有测试样例**，请单击下方控制面板中间的播放按钮。

CPH-NG 会自动向您展开**第一个未通过的已评测测试样例**。

![运行测试样例](images/testCaseRun.png){ width=800 loading=lazy }
/// caption
运行测试样例
///
