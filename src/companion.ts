...
class Companion {
    ...
    constructor(onCreateProblem: (problem: Problem) => void) {
        this.server = createServer((request, response) => {
            ...
            request.on('close', async () => {
                try {
                    const problem = CphCapable.toProblem(JSON.parse(requestData) as CphProblem);
                    const workspaceFolder = vscode.workspace.workspaceFolders?.[0].uri.fsPath;

                    if (!workspaceFolder) {
                        io.info(vscode.l10n.t('No workspace folder found. Please open a workspace folder.'));
                        return;
                    }

                    problem.srcPath = join(workspaceFolder.toString(), this.getProblemFileName(problem.name));
        ...
        this.server.on('error', e => {
            if (vscode.extensions.getExtension('divyanshuagrawal.competitive-programming-helper')?.isActive) {
                io.warn(vscode.l10n.t('CPH-NG is not compatible with CPH, please disable CPH to use CPH-NG.'));
            } else {
                io.error(vscode.l10n.t('Failed to start companion server: {error}.', { error: e.message }));
            }
        });
        ...
        try {
            if (Settings.problem.templateFile) {
                try {
                    ...
                    io.warn(vscode.l10n.t('Failed to use template file: {error}, creating empty file instead', { error: err.message }));
    ...
    private getProblemFileName(name: string) {
        ...
        return (words ? `${words.join("_")}` : `${name.replace(/\W+/g, "_")}`) + '.cpp';
    }
}
