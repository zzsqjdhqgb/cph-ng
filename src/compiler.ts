...
export class Compiler {
    public async getExecutablePath(filePath: string): Promise<string> {
        try {
            ...
            return join(Settings.cache.directory, 'bin', base + (type() === 'Windows_NT' ? '.exe' : ''));
        } catch (error: unknown) {
            const err = error as Error;
            throw new Error(`Failed to generate executable path: ${err.message}`);
        }
    }

    public async compile(filePath: string, outputPath: string, abortController: AbortController): Promise<string> {
        ...
        switch (ext) {
            ...
            case '.c':
                ...
                break;
            default:
                return vscode.l10n.t('Unsupported file type: {type}.', { type: ext });
        }
